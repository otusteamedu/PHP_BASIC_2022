...
/**
 * Вывести в цикле значения массива, в котором в качестве ключей будут использоваться названия областей,
 *  а в качестве значений – массивы с названиями городов из соответствующей области, чтобы результат был таким:
 *      Московская область:
...
function print_region_cities(array $regionCities, ?string $firstLetter = null): void
{
    foreach ($regionCities as $region => $cities) {
        ...
        echo "$region:\n";
        echo join("", array_map(fn ($city) => "\t$city\n", $filteredCities ?? $cities));
        echo "\n";
    }
}
...
// print_region_cities($regionCities, 'К');
