...
class RaceRepo extends Model
{
    ...
    public static function create() {
        if(!empty($_POST['race_name']) && !empty($_POST['race_description']) && !empty($_POST['race_responsibly_user_id']) && !empty($_POST['race_project_id'])) {
            $secure_race_name = htmlspecialchars($_POST['race_name']);
            $secure_race_description = htmlspecialchars($_POST['race_description']);
            $secure_race_responsibly_user_id = htmlspecialchars($_POST['race_responsibly_user_id']);
            $secure_race_racetype_id = htmlspecialchars($_POST['race_racetype_id']);
            // Сделано специально, для того чтобы сделать свой отлов исключений!
            try {
                $allowedExtensions = ['1','2'];
                if(!in_array($secure_race_racetype_id, $allowedExtensions)) {
            ...
            $race->race_responsibly_user_id = $secure_race_responsibly_user_id;
            $race->race_racetype_id = $secure_race_racetype_id;
            try {
                if(!$race->save()) {
